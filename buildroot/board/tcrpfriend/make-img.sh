...
MY_ROOT="${CONFIG_DIR}/.."
IMAGE_FILE="${MY_ROOT}/tcrpfriend.img"
BOARD_PATH="${CONFIG_DIR}/board/tcrpfriend"
...
# Format partitions
sudo mkdosfs -F32 -n TCRPFRIEND1 "/dev/loop8p1"    >/dev/null 2>&1
sudo mkfs.ext2 -F -F -L TCRPFRIEND2 "/dev/loop8p2" >/dev/null 2>&1
sudo mkfs.ext4 -F -F -L TCRPFRIEND3 "/dev/loop8p3" >/dev/null 2>&1
...
echo "Copying files"
sudo cp "${BINARIES_DIR}/bzImage"            "${BINARIES_DIR}/p3/bzImage-TCRPFRIEND"
sudo cp "${BINARIES_DIR}/rootfs.cpio.xz"     "${BINARIES_DIR}/p3/initrd-TCRPFRIEND"
